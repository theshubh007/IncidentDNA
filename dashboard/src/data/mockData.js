...
export const SERVICES = [
    { id: 'llm-gateway', name: 'llm-gateway', status: 'healthy', uptime: 99.97, latency: 42, errorRate: 0.02, dependencies: ['db-primary', 'redis-cluster', 'openai-api'] },
    { id: 'api-gateway', name: 'api-gateway', status: 'healthy', uptime: 99.99, latency: 12, errorRate: 0.01, dependencies: ['llm-gateway', 'auth-service', 'alert-engine'] },
    { id: 'auth-service', name: 'auth-service', status: 'healthy', uptime: 99.95, latency: 38, errorRate: 0.03, dependencies: ['db-primary', 'redis-cluster'] },
    { id: 'alert-engine', name: 'alert-engine', status: 'healthy', uptime: 99.92, latency: 85, errorRate: 0.04, dependencies: ['redis-cluster', 'event-bus'] },
    { id: 'threat-analyzer', name: 'threat-analyzer', status: 'healthy', uptime: 99.88, latency: 67, errorRate: 0.05, dependencies: ['llm-gateway', 'db-primary', 'model-registry'] },
    { id: 'model-registry', name: 'model-registry', status: 'healthy', uptime: 99.91, latency: 55, errorRate: 0.03, dependencies: ['db-replica', 'redis-cluster'] },
];

export const INCIDENTS_DATA = [
    {
        id: 'INC-001', severity: 'critical', service: 'llm-gateway', detected: '2026-02-27T14:23:00Z', status: 'resolved',
        confidence: 0.94, blastRadius: ['threat-analyzer', 'api-gateway'], actionsFired: 3,
        ...
        timeline: [
            ...
            { time: '14:23:12', agent: 'Ag1-Detector', action: 'Blast radius: threat-analyzer, api-gateway at risk (ETA 12m)', status: 'complete' },
        ...
        blastRadiusDetail: {
            primary: 'llm-gateway',
            affected: [
                { service: 'threat-analyzer', impact: 'high', eta: '12m', reason: 'Direct dependency on llm-gateway' },
                { service: 'api-gateway', impact: 'medium', eta: '15m', reason: 'Routes 40% traffic through llm-gateway' },
            ]
        },
        patternMemory: [
            { id: 'INC-012', similarity: 0.92, service: 'llm-gateway', cause: 'DB pool exhaustion during peak', fix: 'Increased pool_max from 50 to 100', date: '2026-01-15' },
            { id: 'INC-008', similarity: 0.87, service: 'threat-analyzer', cause: 'Connection timeout cascade', fix: 'Added circuit breaker with 5s timeout', date: '2026-01-02' },
            { id: 'INC-003', similarity: 0.71, service: 'auth-service', cause: 'Redis connection leak', fix: 'Patched connection pool cleanup', date: '2025-12-18' },
        ],
        actions: [
        ...
        postmortem: {
            summary: 'Payment service experienced DB connection pool exhaustion following deploy sha-a1b2c3d4, causing cascading failures to threat-analyzer and api-gateway.',
    ...
    },
    {
        ...
        confidence: 0.78, blastRadius: ['auth-service'], actionsFired: 2,
        ...
        blastRadiusDetail: {
            primary: 'api-gateway',
            affected: [
                { service: 'auth-service', impact: 'medium', eta: '20m', reason: 'Downstream of api-gateway routing' },
            ]
        },
        patternMemory: [
    ...
    {
        id: 'INC-003', severity: 'critical', service: 'threat-analyzer', detected: '2026-02-26T22:45:00Z', status: 'resolved',
        confidence: 0.91, blastRadius: ['llm-gateway', 'model-registry'], actionsFired: 3,
        ...
        blastRadiusDetail: {
            primary: 'threat-analyzer',
            affected: [
                { service: 'llm-gateway', impact: 'high', eta: '8m', reason: 'Orders queue payment requests' },
                { service: 'model-registry', impact: 'medium', eta: '15m', reason: 'Stock reservation depends on order processing' },
            ]
        },
        patternMemory: [
            { id: 'INC-005', similarity: 0.88, service: 'threat-analyzer', cause: 'Event listener leak', fix: 'Added cleanup in disconnect handler', date: '2025-12-28' },
        ],
        actions: [
    ...
    {
        id: 'INC-004', severity: 'info', service: 'alert-engine', detected: '2026-02-26T08:30:00Z', status: 'resolved',
        ...
        blastRadiusDetail: { primary: 'alert-engine', affected: [] },
        patternMemory: [],
        actions: [
    ...
    {
        id: 'INC-005', severity: 'warning', service: 'model-registry', detected: '2026-02-25T16:10:00Z', status: 'resolved',
        confidence: 0.83, blastRadius: ['threat-analyzer'], actionsFired: 2,
        ...
        blastRadiusDetail: {
            ...
            affected: [
                { service: 'threat-analyzer', impact: 'low', eta: '20m', reason: 'Inventory lookups slower but functional' },
            ]
        },
        patternMemory: [
            { id: 'INC-011', similarity: 0.79, service: 'auth-service', cause: 'Redis failover cold start', fix: 'Pre-warm cache script on failover', date: '2026-01-12' },
        ],
        actions: [
...
export const RELEASES_DATA = [
    {
        id: 'REL-042', service: 'llm-gateway', version: 'v2.14.0', confidence: 34, risk: 'high',
    ...
    {
        id: 'REL-040', service: 'threat-analyzer', version: 'v1.22.0', confidence: 58, risk: 'medium',
    ...
    {
        id: 'REL-039', service: 'alert-engine', version: 'v2.5.1', confidence: 91, risk: 'low',
...
export const AUDIT_LOG_DATA = [
    { actionId: 'ACT-001', decisionId: 'DEC-001', toolkit: 'composio-slack', actionType: 'SLACK_POST_MESSAGE', status: 'sent', retryCount: 0, idempotencyKey: 'INC-001:slack:alert', timestamp: '2026-02-27T14:23:42Z', request: { channel: '#incident-alerts', text: '[P1] llm-gateway: DB pool exhaustion...' }, response: { ok: true, ts: '1709045022.001' } },
    { actionId: 'ACT-002', decisionId: 'DEC-001', toolkit: 'composio-github', actionType: 'GITHUB_CREATE_ISSUE', status: 'created', retryCount: 0, idempotencyKey: 'INC-001:github:issue', timestamp: '2026-02-27T14:23:44Z', request: { repo: 'org/incidents', title: '[INC-001] llm-gateway — DB pool exhaustion' }, response: { number: 347, html_url: 'https://github.com/org/repo/issues/347' } },
    ...
    { actionId: 'ACT-007', decisionId: 'DEC-003', toolkit: 'composio-slack', actionType: 'SLACK_POST_MESSAGE', status: 'sent', retryCount: 1, idempotencyKey: 'INC-003:slack:alert', timestamp: '2026-02-26T22:45:40Z', request: { channel: '#incident-alerts', text: '[P1] threat-analyzer: memory leak...' }, response: { ok: true, ts: '1709001940.003' } },
    { actionId: 'ACT-008', decisionId: 'DEC-003', toolkit: 'composio-github', actionType: 'GITHUB_CREATE_ISSUE', status: 'failed', retryCount: 3, idempotencyKey: 'INC-003:github:issue:attempt1', timestamp: '2026-02-26T22:45:42Z', request: { repo: 'org/incidents', title: '[INC-003] threat-analyzer — memory leak' }, response: { error: 'rate_limited', retry_after: 60 } },
    { actionId: 'ACT-009', decisionId: 'DEC-003', toolkit: 'composio-github', actionType: 'GITHUB_CREATE_ISSUE', status: 'created', retryCount: 0, idempotencyKey: 'INC-003:github:issue', timestamp: '2026-02-26T22:46:44Z', request: { repo: 'org/incidents', title: '[INC-003] threat-analyzer — memory leak' }, response: { number: 345, html_url: 'https://github.com/org/repo/issues/345' } },
...
export const STEPPER_STATES = [
    ...
    { id: 'blast-radius', label: 'Blast Radius', status: 'complete', timestamp: '14:23:12', detail: 'threat-analyzer at risk (ETA 12m), api-gateway (ETA 15m)', evidence: 'SERVICE_DEPENDENCIES graph' },
...
export const DEPENDENCY_GRAPH = {
    nodes: [
        ...
        { id: 'llm-gateway', x: 120, y: 140, status: 'critical' },
        { id: 'auth-service', x: 380, y: 140, status: 'healthy' },
        { id: 'threat-analyzer', x: 60, y: 240, status: 'warning' },
        { id: 'alert-engine', x: 320, y: 240, status: 'healthy' },
        { id: 'model-registry', x: 190, y: 240, status: 'healthy' },
        { id: 'db-primary', x: 120, y: 340, status: 'healthy' },
        { id: 'redis-cluster', x: 300, y: 340, status: 'healthy' },
    ],
    edges: [
        { from: 'api-gateway', to: 'llm-gateway' },
        { from: 'api-gateway', to: 'auth-service' },
        { from: 'api-gateway', to: 'alert-engine' },
        { from: 'llm-gateway', to: 'db-primary' },
        { from: 'llm-gateway', to: 'redis-cluster' },
        { from: 'threat-analyzer', to: 'llm-gateway' },
        { from: 'threat-analyzer', to: 'model-registry' },
        { from: 'threat-analyzer', to: 'db-primary' },
        { from: 'auth-service', to: 'db-primary' },
        { from: 'auth-service', to: 'redis-cluster' },
        { from: 'alert-engine', to: 'redis-cluster' },
        { from: 'model-registry', to: 'redis-cluster' },
    ],
};
...
export const SIMULATION_SCENARIOS = [
    {
        id: 'llm-error-spike',
        label: 'LLM Gateway Error Spike',
        description: 'Simulates a post-deploy error rate spike on llm-gateway (error_rate: 0.02 → 0.22)',
        service: 'llm-gateway',
    ...
    },
    {
        ...
        description: 'Simulates DB connection pool saturation on threat-analyzer',
...
export const SERVICE_SPARKLINES = {
    'llm-gateway': { latency: generateSparkline(24, 42, 8, { index: 20, value: 320 }), errorRate: generateSparkline(24, 0.02, 0.01, { index: 20, value: 0.22 }) },
    'api-gateway': { latency: generateSparkline(24, 12, 3, { index: 18, value: 340 }), errorRate: generateSparkline(24, 0.01, 0.005) },
    'auth-service': { latency: generateSparkline(24, 38, 6), errorRate: generateSparkline(24, 0.03, 0.01) },
    'alert-engine': { latency: generateSparkline(24, 85, 15), errorRate: generateSparkline(24, 0.04, 0.02) },
    'threat-analyzer': { latency: generateSparkline(24, 67, 12, { index: 22, value: 280 }), errorRate: generateSparkline(24, 0.05, 0.02) },
    'model-registry': { latency: generateSparkline(24, 55, 10), errorRate: generateSparkline(24, 0.03, 0.01) },
};
...
export const RUNBOOKS_DATA = [
    { id: 'RB-001', title: 'DB Pool Tuning Guide', service: 'llm-gateway', content: 'When connection pool utilization exceeds 80%, increase pool_max by 50%. Monitor for 15 minutes. If issue persists, check for long-running queries using pg_stat_activity.', lastUsed: '2026-02-27', matchCount: 5 },
